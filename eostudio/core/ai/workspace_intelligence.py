...
class SymbolIndex:
    ...
    def _parse_ts_js(
        ...
        for m in re.finditer(
            r"(?:export\s+)?const\s+(\w+)\s*=\s*(?:async\s*)?\(", content
        ):
            line = content[:m.start()].count("\n") + 1
            symbols.append(Symbol(
                name=m.group(1), kind="function",
...
class WorkspaceIntelligence:
    ...
    def analyze_health(self) -> ArchitectureHealth:
        ...
        large_files = [
            idx.path for idx in self._index.all_files()
            if idx.loc > 500
        ]
        god_classes = [
            sym for sym in self._index.search("", kind="class")
        ...
        test_files = {
            idx.path for idx in self._index.all_files()
            if "test" in idx.path.lower()
        }
        source_files = [
            idx.path for idx in self._index.all_files()
            if "test" not in idx.path.lower()
        ]
        ...
        if circular:
            suggestions.append(
                f"Resolve {len(circular)} circular dependency(ies) to improve maintainability"
            )
        if large_files:
            suggestions.append(
                f"Split {len(large_files)} large file(s) (>500 LOC) into smaller modules"
            )
        if missing_tests:
            suggestions.append(
                f"Add tests for {len(missing_tests)} untested module(s)"
            )
        if god_classes:
            suggestions.append(
                f"Refactor {len(god_classes)} large class(es) using Single Responsibility Principle"
            )
